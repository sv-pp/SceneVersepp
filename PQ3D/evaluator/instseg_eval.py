...
class InstSegEval(BaseEvaluator):
    def __init__(self, cfg, accelerator, dataloaders=None, **kwargs):
        ...
        if self.save:
            self.eval_results = []
            dataset_name = cfg.data.get(cfg.mode, ["train"])[0]
            self.save_dir = Path(cfg.exp_dir) / "eval_results" / dataset_name / "InstSeg"
    ...
    def record(self):
        ...
        ap_results = {}
        if self.save:
            np.save(self.save_dir / f"bbox_preds.npy", self.bbox_preds)
            np.save(self.save_dir / f"bbox_gt.npy", self.bbox_gt)
            np.save(self.save_dir / f"preds.npy", self.preds)
    ...
    #         # === STEP 3: GT BOX 计算（稀疏 full_masks） ===
    #         bbox_data = []
    #         full_masks_sparse = full_masks[bid]  # ✅ 每个 batch 的稀疏矩阵
    #         n_inst = full_masks_sparse.shape[0]

    #         for obj_id in range(n_inst):
    #             if instance_labels[bid][obj_id].item() == self.ignore_label:
    #                 continue

    #             # ✅ 获取该实例的点索引（稀疏矩阵行）
    #             start = full_masks_sparse.indptr[obj_id]
    #             end = full_masks_sparse.indptr[obj_id + 1]
    #             point_indices = full_masks_sparse.indices[start:end]

    #             if point_indices.size == 0:
    #                 continue

    #             obj_coords = raw_coordinates[bid][point_indices, :]
    #             if obj_coords.shape[0] > 0:
    #                 obj_center = obj_coords.mean(axis=0)
    #                 obj_axis_length = obj_coords.max(axis=0) - obj_coords.min(axis=0)
    #                 bbox = np.concatenate((obj_center, obj_axis_length))
