...
if __name__ == "__main__":
    ...
    with open(args.data_file, 'r') as f:
        data = json.load(f)
    data_root = Path(args.data_file).parent
    point_cloud_files = []
    for d in data:
        point_cloud_files.append(str(data_root / d['point_clouds'][0]))

    for point_cloud_file in tqdm(point_cloud_files):
        # load the point cloud
        if not os.path.exists(point_cloud_file):
            print(f"⚠️ {point_cloud_file} not found, skipped")
            continue
        output_filename = os.path.basename(point_cloud_file).replace(".ply", ".txt")
        if os.path.exists(os.path.join(args.output, output_filename)):
            continue
