...
def save_results(table_data, title, args):
    """save evaluation results"""
    table = AsciiTable(table_data)
    table.title = title
    result_str = "\n" + table.table

    result_dir = os.path.dirname(args.pred_dir)
    pred_name = os.path.basename(args.pred_dir.rstrip("/"))
    save_path = os.path.join(result_dir, f"{pred_name}.txt")
...
if __name__ == "__main__":
    ...
    scene_id_list = os.listdir(args.pred_dir)
    scene_id_list = [d.split('.')[0] for d in scene_id_list]
    ...
    for class_name in OBJECTS:
        try:
            ...
            f1_25_score = round(f1_25 * 100, 3)
            f1_50_score = round(f1_50 * 100, 3)
            table_data.append([class_name, f1_25_score, f1_50_score])
            all_f1_25.append(f1_25_score)
            all_f1_50.append(f1_50_score)
        except:
            continue
    mean_f1_25 = round(np.mean(all_f1_25), 3)
    mean_f1_50 = round(np.mean(all_f1_50), 3)
    table_data.append(["MEAN", mean_f1_25, mean_f1_50])

    print("\n" + AsciiTable(table_data).table)
    save_results(table_data, "Objects", args)