...
class SpatialLMQwenForCausalLM(Qwen2ForCausalLM):
    ...
    def __init__(self, config):
        ...
        #         "Qwen/Qwen2-0.5B",  # 或者换成对应的 checkpoint
        #         config=config,
        #         trust_remote_code=True
        #     )
        #     logger.info("Loaded Qwen2 pretrained weights for self.model")
        # except Exception as e:
        #     # 如果下载失败，就 fallback 到随机初始化
        #     logger.warning(f"Could not load pretrained Qwen2, using random init. Error: {e}")
        #     self.model = Qwen2Model(config)
